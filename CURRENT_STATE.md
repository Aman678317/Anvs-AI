...
1. **Invariant #1: Multi-Tenant Isolation & Crypto Key Segregation**
   - *Status*: **PASS**. Enforced via PostgreSQL RLS policies (`current_setting('app.current_tenant_id', true)`), tenant-scoped Redis Stream keys (`meeting:{id}:*`), and AES-256-GCM authenticated encryption.
2. **Invariant #2: Immutable Speech Lineage Provenance**
   - *Status*: **PASS**. `SourceSegmentEvent` serves as the authoritative root. All translations carry `source_segment_id`. No translation or assistant output can loop back into source STT.
3. **Invariant #3: 20 kHz Ultrasonic Watermark Audio Loop Rejection**
   - *Status*: **PASS**. Injected by `services/tts_worker/engine.py` using `embed_watermark` at 20,000 Hz. Checked and rejected by `packages/audio/ingestion.py` in `process_frame`.
4. **Invariant #4: At-Least-Once Delivery with Poison-Pill DLQ Quarantine**
   - *Status*: **PASS**. `DLQRetryManager` in `services/orchestrator/dlq_retry.py` enforces maximum 3 retry attempts with exponential backoff and quarantine to DLQ stream.
